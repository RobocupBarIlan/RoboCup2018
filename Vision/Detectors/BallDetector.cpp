...
//			cv::addWeighted(frame, 1.5, blurred_frame, -0.5, 0, frame);
	...
	BallDetector::FieldCalibration(hsv_channels[0], field_min_hue,
			field_max_hue);
	...
	for (int i = 0; i < frame.rows; i++) {
		for (int j = 0; j < frame.cols; j++) {
			...
			if (hsv_channels[0].at<uchar>(i, j) >= field_min_hue
					&& hsv_channels[0].at<uchar>(i, j) <= field_max_hue
					&& hsv_channels[1].at<uchar>(i, j)
							> MAX_WHITE_SATURATION / 2.0) {
				field_mat.at<uchar>(i, j) = 255; //If in range (i.e- field pixel) set to 0.
			}
			//Check saturation& value against bounds:

			if (hsv_channels[1].at<uchar>(i, j) <= MAX_WHITE_SATURATION
					&& hsv_channels[2].at<uchar>(i, j) >= MIN_WHITE_VALUE) {
...
//			waitKey(1);
	...
	BallDetector::CalculateBoundingHorizontalLine(field_mat,
			bounding_horizontal_line);
...
//			waitKey(1);
	...
	cv::addWeighted(frame_luminance, 2, blurred_luminance, -1, 0,
			frame_luminance);
...
//			  waitKey(10);
	...
	CircleFitRansac(frame, frame_edges_clone, found_circles,
			MIN_BALL_DIAMETER / 2, MAX_BALL_DIAMETER / 2);
	...
	if (found_circles.size() == 0) //No candidate found:
		...
		if (IsFirstRun) {
			cout
					<< "Can\'t find any ball candidate for calibration! program is terminated."
					<< endl;
		}
		//No ball found:
		returned_center.x = -1;
	...
	std::sort(found_circles.begin(), found_circles.end(),
			BallCandidateRansac::by_support());
	...
	if (IsFirstRun) //Ask the user which part of the image is the ball - so we have hue histogram of colors for the ball:
	{
		...
		for (int i = 0; i < found_circles.size(); i++) //For each circle candidate - ask the user if it is the ball:
			...
			BallDetector::GetCircleBoundingRectangleInFrame(
					found_circles[i].m_center, found_circles[i].m_radius,
					frame.rows, frame.cols, candidate_bounding_rect);
			if (candidate_bounding_rect.width != -1) //If ball center is in frame (o.w can't calibrate the ball's colors):
			{
				...
				while (userInput != 'y' && userInput != 'n') {
					cout << "You must enter \'y\' for yes or \'n\' for no !"
							<< endl;
					cout
							<< "Does the following image (\"candidate\" window) shows the ball and only the ball? [enter y for \'yes\' and n for \'no\']"
							<< endl;
					cin >> userInput;
				}
		...
		BallDetector::BallHistogramCalibration(hsv_channels[0],
				candidate_bounding_rect, field_min_hue, field_max_hue);
		IsFirstRun = false;
	} else //If not first run - ball's calibration is already done - we can check which candidate is the ball:
	{
		...
		for (uint i = 0; i < found_circles.size(); i++) //For each circle (from the most supported to the least - check the colors correlation):
			...
			BallDetector::GetCircleBoundingRectangleInFrame(
					found_circles[i].m_center, found_circles[i].m_radius,
					frame.rows, frame.cols, candidate_bounding_rect);
			if (candidate_bounding_rect.width != -1
					&& candidate_bounding_rect.width > 0) //If candidate's center is in frame - compute the colors correlation.
							{

				candidate = hsv_channels[0];
				candidate = candidate(candidate_bounding_rect);

				BallDetector::CountNumFieldPixels(candidate, field_min_hue,
						field_max_hue, num_field_pixels);
				percentage_field_pixels = num_field_pixels
						/ (0.0 + candidate.rows * candidate.cols);

				//field_min_hue=0;
				//field_max_hue=0;
				BallDetector::CalculateBallColorsCorrelation(hsv_channels[0],
						candidate_bounding_rect, correlation, field_min_hue,
						field_max_hue);

				if (correlation >= COLORS_CORRELATION_THRESHOLD
						&& percentage_field_pixels <= FIELD_PIXELS_THRESHOLD) {
					...
					circle(frame, found_circles[i].m_center,
							found_circles[i].m_radius, Scalar(0, 0, 255), 2);
...
void BallDetector::FieldCalibration(Mat& hue_matrix, uchar& field_min_hue,
		uchar& field_max_hue) {
	...
	for (uint i = 0; i < hue_matrix.rows; i++) {
		for (uint j = 0; j < hue_matrix.cols; j++) {
			hue_histogram[hue_matrix.at<uchar>(i, j)] =
					hue_histogram[hue_matrix.at<uchar>(i, j)] + 1;
		}
	}
	//Sum all pixels to get total_num_of_green_pixels.
	for (uint k = 0; k < HUE_DISCRETIZATION; k++) {
		total_num_of_green_pixels = total_num_of_green_pixels
				+ hue_histogram[k];
	}
	if (total_num_of_green_pixels
			< NOT_ENOUGH_GREENS_IN_IMAGE_THRESHOLD * TOTAL_NUM_OF_PIXELS) //If not enough green pixels in image:
					{
		cout
				<< "Not enough green pixels in image to do field green calibration!"
				<< endl;
	...
	while (sum_of_green_pixels_in_histogram / (total_num_of_green_pixels + 0.0)
			< (1 - PERCENTAGE_THRESHOLD) / 2) {
		i++;
		sum_of_green_pixels_in_histogram = sum_of_green_pixels_in_histogram
				+ hue_histogram[i];
	}
...
void BallDetector::CalculateBoundingHorizontalLine(Mat& field_mat,
	...
	for (int i = field_mat.rows - 1; i > 0; i--) {
		for (int j = 0; j < field_mat.cols; j++) {
			sum_of_rows_vector[i] = sum_of_rows_vector[i]
					+ ((field_mat.at<uchar>(i, j) > 0) ? 1 : 0);
		}
		total_num_of_field_pixels = total_num_of_field_pixels
				+ sum_of_rows_vector[i]; //Sum of sum of rows = total num. of field pixels.
	}
	...
	while (num_of_field_pixels_below_bounding_line
			/ (total_num_of_field_pixels + 0.0) < PERCENTAGE_THRESHOLD) {
		//cout << "row_num:" << row_num << "," << "proportion:" << to_string(num_of_field_pixels_below_bounding_line / (total_num_of_field_pixels + 0.0)) << endl;
		row_num--;
		num_of_field_pixels_below_bounding_line =
				num_of_field_pixels_below_bounding_line
						+ sum_of_rows_vector[row_num]; //add the row above.
	}
 ...
 */
void BallDetector::CircleFitKasa(vector<Point>& contour, Point& center,
		double& radius) {
	if (contour.size() >= 3) //O.w we can't fit
		...
		for (uint i = 0; i < contour.size(); i++) //Init matrices:
			...
			Y.at<double>(i, 0) = pow((contour[i].x / normalization_factor), 2)
					+ pow((contour[i].y / normalization_factor), 2);
		}
		...
		radius = sqrt(
				c.at<double>(2, 0) + pow(0.5 * c.at<double>(0, 0), 2)
						+ pow(0.5 * c.at<double>(1, 0), 2));
 ...
 */
void BallDetector::CircleFitRansac(Mat& frame, Mat& frame_edges,
		vector<BallCandidateRansac>& found_circles, int min_radius,
		int max_radius) {
...
//		cout<<"edge_points:"<<edge_points.size()<<endl;
		...
		for (int i = 0; i < NUM_ITERATIONS; i++) {
			...
			while (reshuffles_count < MAX_RESHUFFLES
					&& (random_point_index_2 == random_point_index_1
							|| sqrt(
									pow(
											edge_points[random_point_index_1].x
													- edge_points[random_point_index_2].x,
											2)
											+ pow(
													edge_points[random_point_index_1].y
															- edge_points[random_point_index_2].y,
													2)) > MAX_BALL_DIAMETER)) {
				random_point_index_2 = rand() % edge_points.size();
				reshuffles_count++;
			}
			...
			reshuffles_count = 0;
			while (reshuffles_count < MAX_RESHUFFLES
					&& (random_point_index_3 == random_point_index_1
							|| sqrt(
									pow(
											edge_points[random_point_index_1].x
													- edge_points[random_point_index_3].x,
											2)
											+ pow(
													edge_points[random_point_index_1].y
															- edge_points[random_point_index_3].y,
													2)) > MAX_BALL_DIAMETER
							|| sqrt(
									pow(
											edge_points[random_point_index_3].x
			...
			chosen_3_points.push_back(
					Point(edge_points[random_point_index_1].x,
							edge_points[random_point_index_1].y));
			chosen_3_points.push_back(
					Point(edge_points[random_point_index_2].x,
							edge_points[random_point_index_2].y));
			chosen_3_points.push_back(
					Point(edge_points[random_point_index_3].x,
							edge_points[random_point_index_3].y));

			BallDetector::CircleFitKasa(chosen_3_points, checked_circle_center,
					checked_circle_radius);

			//check only circles with searched radius:
			if (checked_circle_radius >= min_radius
					&& checked_circle_radius <= max_radius) {
				num_supporting_points = 0;
				for (uint j = 0; j < edge_points.size(); j++) //Search for circle supporting points:
						{
					dist_from_center = sqrt(
							pow(checked_circle_center.x - edge_points[j].x, 2)
									+ pow(
											checked_circle_center.y
													- edge_points[j].y, 2));
					//cout<<"dist:"<<dist_from_center<<endl;
					if (dist_from_center
							>= checked_circle_radius - THRESHOLD_DISTANCE
							&& dist_from_center
									<= checked_circle_radius
											+ THRESHOLD_DISTANCE) {
				...
				if (num_supporting_points > SUPPORT_THRESHOLD) //Add the circle candidate to the vectors of centers and radii:
						{
					found_circles.push_back(
							BallCandidateRansac(
									Point(checked_circle_center.x,
											checked_circle_center.y),
									cvRound(checked_circle_radius),
									num_supporting_points));
				}
 ...
 */
void BallDetector::GetCircleBoundingRectangleInFrame(Point& center, int& radius,
		int& frame_rows, int& frame_cols, Rect& bounding_rect) {
	int left, top, width, height;
	if (center.x > 0 && center.x < frame_cols && center.y > 0
			&& center.y < frame_rows) //If circle's center is in frame:
					{
 ...
 */
void BallDetector::BallHistogramCalibration(Mat& hue_mat, Rect& ball_roi,
		uchar& field_min_hue, uchar& field_max_hue) {
	...
	for (int i = 0; i < HUE_QUANTIZATION; i++) {
		ball_hue_histogram[i] = ball_hue_histogram[i]
				/ (ball_hue_roi.rows * ball_hue_roi.cols);
	}
 ...
 */
void BallDetector::CalculateBallColorsCorrelation(Mat& hue_mat,
		Rect& candidate_bounding_rect, double& correlation,
		uchar& field_min_hue, uchar& field_max_hue) {
	...
	for (int i = 0; i < candidate_hue.rows; i++) {
		...
		for (int j = 0; j < candidate_hue.cols; j++) {
			if (candidate_hue.at<uchar>(i, j) < field_min_hue
					|| candidate_hue.at<uchar>(i, j) > field_max_hue) //Don't count the field colors into the colors histogram.
	...
	for (int i = 0; i < HUE_QUANTIZATION; i++) {
		candidate_hue_histogram[i] = candidate_hue_histogram[i]
	...
	for (int i = 0; i < HUE_QUANTIZATION; i++) {
		sqrt_sum_squares = sqrt_sum_squares
				+ pow(candidate_hue_histogram[i], 2);
	}
	sqrt_sum_squares = sqrt(sqrt_sum_squares);
...
void BallDetector::CountNumFieldPixels(Mat& candidate, uchar& field_min_hue,
		uchar& field_max_hue, uint& num_field_pixels) {
	num_field_pixels = 0; //Init.
	for (int i = 0; i < candidate.rows; i++) {
		for (int j = 0; j < candidate.cols; j++) {
			if (candidate.at<uchar>(i, j) >= field_min_hue
					&& candidate.at<uchar>(i, j) <= field_max_hue) {
				num_field_pixels++;
			}
		}
	}
...
void BallDetector::CalculateDistanceToBall(int& radius,
		double& calculated_distance) {
	calculated_distance = (597.3040 - 18.6316 * radius + 0.236 * pow(radius, 2)
			- 0.001 * pow(radius, 3));
}
